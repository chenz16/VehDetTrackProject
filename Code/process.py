...
def color_hist(img, nbins=32, bins_range=(0, 256)):
    ...
    #color_max = np.amax(img) # test if image is normamized between 0-1
    #if color_max <=1:
        #bins_range = (0,1)
...
def extract_features(imgs, color_space='RGB', spatial_size=(32, 32),
    ...
    for file in imgs:
        ...
        # if if its png format, convert to normal RGB format
...
def draw_labeled_bboxes(img, labels):
    ...
    for car_number in range(1, labels[1]+1):
        ...
        x1 = bbox[1][0]
        y1 = bbox[1][1]
        if np.sqrt((x0-x1)**2 + (y0-y1)**2) >= 64 and np.absolute (x0-x1)> 20 and np.absolute(y0-y1) > 20:
